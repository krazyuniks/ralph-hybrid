...
2. **Propose skills to generate:**
   ...
   3. playwright-visual-regression
      - Callback for screenshot comparison
...
| Visual validation callback | `ralph-hybrid/templates/callbacks/` | Playwright screenshot diff template |
...
├── .ralph-hybrid/{feature}/callbacks/     ← Generated by ralph-plan
...
# Callbacks (extend or replace project/global callbacks)
callbacks:
  post_iteration:
    - .ralph-hybrid/{feature}/callbacks/visual-regression.sh
  pre_commit:
    - .ralph-hybrid/{feature}/callbacks/lint-check.sh
...
Callback that starts browser before iteration, script queries it:
```yaml
# callbacks/pre_iteration.sh
playwright open http://localhost:8010 --headed &
export BROWSER_PID=$!

# callbacks/post_iteration.sh
kill $BROWSER_PID
```
...
**Key insight:** We discover skill/callback/script requirements during feature work. These learnings must be synthesized back into project-level assets—not simply moved, but holistically evaluated and assimilated.
...
│   ├── callbacks/                        # Battle-tested automation
...
└── .ralph-hybrid/
    ...
    └── {feature}/                    # Feature workspace (temporary)
        ...
        ├── callbacks/                    # Discovered during this feature
...
- Which scripts/skills/callbacks were created this feature?
...
**Output:** Template library created with skills, scripts, and callbacks.
...
3. **Callback Template:** `templates/callbacks/post-iteration-visual-diff.sh`
...
4. **ralph-plan Enhancement:** Added `Phase 2.5: ANALYZE`
   ...
   - Proposes skills/scripts/callbacks based on epic type
...
| Pre-iteration visual diff callback | P2 | **Complete** | `templates/callbacks/post-iteration-visual-diff.sh` |
...
- **Deprioritized:** MCP browser callbacks (curl + Playwright tests sufficient)
...
3. Everything should be overridable per feature:
   ...
   - completion_patterns
   - callbacks (can extend or replace)
   - prompt_template
...
### 3. Visual Diff Callback Template (P2)

Create: templates/callbacks/post-iteration-visual-diff.sh
